...
const news = [
 ...
 const newsCard = news.map(n => createNewsCard(n));
 document.body.append(...newsCard);


function createNewsCard(news){
    const newsCard= document.createElement('ul');
newsCard.classList.add('createUl');



const newsTitle = createNewsTitle(news);
const newsHeaderBg= createNewsHeaderBg(news);
const newsCategory= createNewsCategory(news);
const newsBody= createNewsBody(news);
const newsDate= createNewsDate(news);
const liNewsIcon= createliNewsIcon();
const newsBtnLi= createNewsBtnLi();
newsCard.append(newsTitle, newsHeaderBg, newsCategory, newsBody, newsDate, liNewsIcon, newsBtnLi);
return newsCard;
}
...
function createNewsHeaderBg({headerBgSrc, category}){
    const newsHeaderBg= document.createElement('li');
newsHeaderBg.classList.add('headerBgSrc');
const newsImg= document.createElement('img');
newsImg.src= headerBgSrc;
newsImg.alt= category;
newsImg.classList.add('newsImg');
newsHeaderBg.append(newsImg);
function  changeBgLi(){
    newsHeaderBg.classList.toggle('bgLiClick');
}
newsHeaderBg.addEventListener('click', changeBgLi);
return newsHeaderBg
}


function createNewsCategory({category}){
    const newsCategory= document.createElement('li');
newsCategory.textContent= category;
newsCategory.classList.add('category');
function  changeBgLi(){
    newsCategory.classList.toggle('bgLiClick');
}
newsCategory.addEventListener('click', changeBgLi);
return newsCategory;
}
...
newsBody.classList.add('bodyUl');
function  changeBgLi(){
    newsBody.classList.toggle('bgLiClick');
}
newsBody.addEventListener('click', changeBgLi);
return newsBody;
}


function createNewsDate({date}){
    const newsDate= document.createElement('li');
newsDate.textContent=date;
newsDate.classList.add('date');
function  changeBgLi(){
    newsDate.classList.toggle('bgLiClick');
}
newsDate.addEventListener('click', changeBgLi);
return newsDate;
}


function createliNewsIcon(){
    const liNewsIcon= document.createElement('li');
liNewsIcon.classList.add('newsIcon')


const newsIcon= document.createElement('i');
newsIcon.classList.add('fa-solid');
newsIcon.classList.add('fa-heart');
newsIcon.classList.add('icon');
liNewsIcon.append(newsIcon);
function lightClickHeandler(){
       newsIcon.classList.toggle('heandlerLike');
}
newsIcon.addEventListener('click', lightClickHeandler);
return liNewsIcon
}


function createNewsBtnLi(){
    const newsBtnLi= document.createElement('li');
const newsBtn= document.createElement('button');
newsBtn.textContent='Delete'
newsBtn.classList.add('newsBtn');
newsBtnLi.append(newsBtn);

function deleteNewsCard(e){
    e.target.closest('.createUl').remove();
}
newsBtn.addEventListener('click', deleteNewsCard);
function  changeBgLi(){
    newsBtnLi.classList.toggle('bgLiClick');
}
newsBtnLi.addEventListener('click', changeBgLi);
return newsBtnLi
}
